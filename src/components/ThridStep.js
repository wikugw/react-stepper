...
const ThridStep = () => {
  const { currentID, changeValue, currentData, decrementStep, submitData } = useContext(multiStepContext);
  return (
    <div>
      <div>
      ...
      <div>
        ...
        <Button variant="contained" onClick={submitData} color="primary">{currentID ? 'Update' : 'Submit'}</Button>
      </div>
    </div>
  )
}
